...
export class SongCacheService {
  ...
  cacheSong(songId: string, audioUrl: string): Observable<void> {
    return this.http.get(audioUrl, { responseType: 'blob' }).pipe(
      ...
      catchError(error => { console.error('Failed to cache song:', error); throw error; })
    );
  }
  ...
  getSong(songId: string, fallbackUrl: string): Observable<{ blob: Blob; fromCache: boolean }> {
    return this.cache.get(songId).pipe(
      ...
      catchError(() =>
        this.http.get(fallbackUrl, { responseType: 'blob' })
          .pipe(map(blob => ({ blob, fromCache: false })))
      )
    );
  }
  ...
  isCached(songId: string) { return this.cache.has(songId); }
  removeFromCache(songId: string) { return this.cache.delete(songId); }
  ...
  downloadSong(songId: string, songName: string, audioUrl: string): Observable<void> {
    return this.http.get(audioUrl, { responseType: 'blob' }).pipe(
      ...
      map(() => void 0),
      catchError(error => { console.error('Failed to download song:', error); throw error; })
    );
  }
