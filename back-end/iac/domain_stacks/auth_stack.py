...
class AuthStack(Stack):
    def __init__(self, scope: Construct, id: str, cognito_stack: CognitoStack, dynamo_db: DynamoDbStack, s3: S3Stack, shared_layers_stack: SharedLayerStack,
        ...
        self._create_lambdas(dynamo_db, s3, shared_layers_stack, auth_layer_stack, utils_layer_stack, env_vars)
        self._attach_to_api(api_stack)

    def _create_lambdas(self, dynamo_db, s3, shared_layer_stack, auth_layer_stack, utils_layer_stack, env_vars):
        lambda_defs = {
        ...
        for key, path in lambda_defs.items():
            self.lambdas[key] = LambdaWithPermissions(self, key, path, env_vars, dynamo_db, s3, shared_layer_stack, auth_layer_stack, utils_layer_stack).fn
