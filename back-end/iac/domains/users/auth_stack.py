...
class AuthStack(NestedStack):
    def __init__(self, scope: Construct, stack_id: str, cognito_stack: CognitoStack, dynamo_db: DynamoDbStack, s3: S3Stack, libs_layers_stack: LibsLayerStack,
                 auth_layer_stack: AuthLayerStack, utils_layer_stack: UtilsLayerStack, api_stack: ApiGatewayStack, environment: dict, branch: str, **kwargs):
        super().__init__(scope, stack_id, **kwargs)
        self.lambdas = {}
        self.cognito_stack = cognito_stack
        self._create_lambdas(dynamo_db, s3, libs_layers_stack, auth_layer_stack, utils_layer_stack, environment, branch)
